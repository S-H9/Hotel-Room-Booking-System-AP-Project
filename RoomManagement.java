...
//    }
    ...
    public static JPanel createMainPanel() {
        ...
        addLayout.putConstraint(SpringLayout.WEST, editButton, 350, SpringLayout.WEST, addPanel);
        addLayout.putConstraint(SpringLayout.NORTH, editButton, 30, SpringLayout.SOUTH, availableYes);
        addPanel.add(editButton);

        // Add the edit button action listener
        editButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                String currentRoomNumber = (String) roomsTable.getValueAt(selectedRow, 0);
                String currentRoomType = (String) roomsTable.getValueAt(selectedRow, 1);
                String currentPrice = (String) roomsTable.getValueAt(selectedRow, 2);
                String currentAvailability = (String) roomsTable.getValueAt(selectedRow, 3);

                // Create edit dialog
                JDialog editDialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(mainPanel), "Edit Room", true);
                ...
                editDialog.add(new JLabel("Room Number:"), gbc);
                JTextField editRoomNumberField = new JTextField(currentRoomNumber, 15);
                editRoomNumberField.setEnabled(false); // Room number cannot be edited
                gbc.gridx = 1;
                editDialog.add(editRoomNumberField, gbc);

                gbc.gridx = 0; gbc.gridy = 1;
                editDialog.add(new JLabel("Room Type:"), gbc);
                JComboBox<String> editRoomTypeCombo = new JComboBox<>(new String[]{"Single", "Double", "Suite"});
                editRoomTypeCombo.setSelectedItem(currentRoomType);
                gbc.gridx = 1;
                editDialog.add(editRoomTypeCombo, gbc);

                gbc.gridx = 0; gbc.gridy = 2;
                editDialog.add(new JLabel("Price:"), gbc);
                JTextField editPriceField = new JTextField(currentPrice, 15);
                gbc.gridx = 1;
                editDialog.add(editPriceField, gbc);

                gbc.gridx = 0; gbc.gridy = 3;
                editDialog.add(new JLabel("Availability:"), gbc);
                JComboBox<String> editAvailabilityCombo = new JComboBox<>(new String[]{"Available", "Not Available"});
                editAvailabilityCombo.setSelectedItem(currentAvailability);
                gbc.gridx = 1;
                editDialog.add(editAvailabilityCombo, gbc);
                ...
                updateButton.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        String roomNumber = editRoomNumberField.getText();
                        String roomType = (String) editRoomTypeCombo.getSelectedItem();
                        String price = editPriceField.getText();
                        boolean availability = editAvailabilityCombo.getSelectedItem().equals("Available");

                        // Validate input
                        if (roomType.isEmpty() || price.isEmpty()) {
                        ...
                        String sql = "UPDATE rooms SET room_type = ?, price = ?, availability = ? WHERE room_number = ?";
                        ...
                        try (Connection connection = DriverManager.getConnection(url, username, password);
                            ...
                            if (rowsUpdated > 0) {
                                JOptionPane.showMessageDialog(editDialog, "Room details updated successfully!");
                                refreshTableData(roomsTable);
                                refreshTableData(searchResultsTable);
                                editDialog.dispose();
                            } else {
                                JOptionPane.showMessageDialog(editDialog, "Failed to update room details.");
                            }
